...
import kr.co.camera.databinding.FragmentLongHeadBinding
import kr.co.camera.etc.Dlog
import kr.co.camera.etc.extension.getDrawableById

class LongHeadFragment: BaseFragment<FragmentLongHeadBinding>() {

    companion object {
        const val ARG_POSITION = "ARG_POSITION"
        fun newInstance(position: Int): LongHeadFragment = LongHeadFragment().apply {
    ...
    override val layoutRes: Int
        get() = R.layout.fragment_long_head
    ...
    override fun setView(view: View?, savedInstanceState: Bundle?, arguments: Bundle?) {
        ...
        getBinding().longHeadIvHead.background = context?.getDrawableById(R.drawable.dog_02_guide_red)

        val textData = "밝은 곳에서 눈과 코 사이의 각도,\n사각 박스 안 콧구멍과 가운데 선을 맞춘 후\n초점을 맞춰 촬영해 주세요."
        getBinding().longHeadTv.text = textData
                ...
                if (isActive) {
                    ...
                    when {
                        count > 2 -> {
                            ...
                            getBinding().longHeadIvHead.visibility = View.GONE
                        }
                        count > 0 -> {
                            hdCountDown?.sendEmptyMessageDelayed(0, 100)
                            getBinding().longHeadIvHead.visibility = View.VISIBLE
                        }
                        else -> {
                            getBinding().longHeadIvHead.visibility = View.GONE
                            startCountPerOne()
    ...
    private fun startCountPerOne() {
        ...
        hdCountPerOne.postDelayed(object : Runnable {
            override fun run() {
                Dlog.i("결과 : 카운트 다운 : $count")
                getBinding().longHeadIvHead.visibility = if (count == 0 || count%2 == 0) View.VISIBLE else View.GONE
                count++
